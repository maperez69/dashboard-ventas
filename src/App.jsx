...
import { TrendingUp, Award, ShoppingBag, DollarSign, UploadCloud, Filter } from 'lucide-react';

// Datos pre-cargados basados en el análisis real de los archivos de Odoo
const MOCK_DATA = [
  { mes: 'Noviembre', sucursal: 'Buenavista', linea: 'Medicina Estética', vendedora: 'Mar Campos', precioTotal: 450000, cantidad: 120 },
  { mes: 'Noviembre', sucursal: 'Buenavista', linea: 'Aparatología', vendedora: 'Daniela Perez', precioTotal: 250000, cantidad: 80 },
  { mes: 'Noviembre', sucursal: 'Masaryk', linea: 'Medicina Estética', vendedora: 'Paulina', precioTotal: 300000, cantidad: 95 },
  { mes: 'Noviembre', sucursal: 'Masaryk', linea: 'Farmacia', vendedora: 'Ana Gabriela', precioTotal: 294981, cantidad: 200 },
  
  { mes: 'Diciembre', sucursal: 'Buenavista', linea: 'Medicina Estética', vendedora: 'Mar Campos', precioTotal: 400000, cantidad: 110 },
  { mes: 'Diciembre', sucursal: 'Buenavista', linea: 'Aparatología', vendedora: 'Daniela Perez', precioTotal: 200000, cantidad: 70 },
  { mes: 'Diciembre', sucursal: 'Masaryk', linea: 'Inyectables', vendedora: 'Paulina', precioTotal: 280000, cantidad: 85 },
  { mes: 'Diciembre', sucursal: 'Masaryk', linea: 'Cosmetología', vendedora: 'Ana Gabriela', precioTotal: 208444, cantidad: 150 },
  
  { mes: 'Enero', sucursal: 'Buenavista', linea: 'Medicina Estética', vendedora: 'Mar Campos', precioTotal: 173614, cantidad: 60 },
  { mes: 'Enero', sucursal: 'Buenavista', linea: 'Aparatología', vendedora: 'Daniela Perez', precioTotal: 150000, cantidad: 50 },
  { mes: 'Enero', sucursal: 'Masaryk', linea: 'Medicina Estética', vendedora: 'Paulina', precioTotal: 200000, cantidad: 75 },
  { mes: 'Enero', sucursal: 'Masaryk', linea: 'Farmacia', vendedora: 'Ana Gabriela', precioTotal: 137505, cantidad: 120 },
];

const COLORES_PIE = ['#0ea5e9', '#3b82f6', '#8b5cf6', '#ec4899', '#f43f5e', '#f97316'];
...
export default function App() {
  ...
  // Cálculos de KPIs
  const kpis = useMemo(() => {
    const total = datosFiltrados.reduce((acc, curr) => acc + (curr.precioTotal || 0), 0);
    const cantidad = datosFiltrados.reduce((acc, curr) => acc + (curr.cantidad || 0), 0);
    
    // Calcular mejor vendedora
    const vendedoras = {};
    datosFiltrados.forEach(item => {
      vendedoras[item.vendedora] = (vendedoras[item.vendedora] || 0) + item.precioTotal;
    });
    const topVendedora = Object.keys(vendedoras).sort((a, b) => vendedoras[b] - vendedoras[a])[0] || 'N/A';
  ...
  const datosEvolucion = useMemo(() => {
    const agrupado = {};
    datosFiltrados.forEach(item => {
      if (!agrupado[item.mes]) agrupado[item.mes] = { mes: item.mes, Buenavista: 0, Masaryk: 0, Total: 0 };
      if (item.sucursal.includes('Buenavista')) agrupado[item.mes].Buenavista += item.precioTotal;
      else if (item.sucursal.includes('Masaryk')) agrupado[item.mes].Masaryk += item.precioTotal;
  ...
  const datosVendedoras = useMemo(() => {
    const agrupado = {};
    datosFiltrados.forEach(item => {
      agrupado[item.vendedora] = (agrupado[item.vendedora] || 0) + item.precioTotal;
    });
    return Object.entries(agrupado)
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 font-sans text-slate-800">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Tribune Tech</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 lg:col-span-2">
          <h3 className="text-lg font-semibold mb-4 text-slate-800">Evolución de Ventas por Mes</h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={datosEvolucion} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <Line type="monotone" dataKey="Buenavista" stroke="#8b5cf6" strokeWidth={2} dot={{ r: 4 }} />
                <Line type="monotone" dataKey="Masaryk" stroke="#f43f5e" strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <h3 className="text-lg font-semibold mb-4 text-slate-800">Ventas por Línea</h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
      ...
      {/* TABLA DE VENDEDORAS */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="text-lg font-semibold mb-4 text-slate-800">Top 5 Vendedoras</h3>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={datosVendedoras} layout="vertical" margin={{ top: 5, right: 30, left: 60, bottom: 5 }}>
...
function KpiCard({ titulo, valor, icono, bgIcon }) {
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
      <div className={`${bgIcon} w-14 h-14 rounded-full flex items-center justify-center shrink-0`}>
        {icono}
      </div>
      <div>
